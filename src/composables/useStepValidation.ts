...
export function useStepValidation() {
  ...
  const validateStep = (stepNumber: number): boolean => {
    try {
      switch (stepNumber) {
        ...
        case 2:
          return validatePropertyDetails(formStore.propertyForm.propertyDetail);
        case 3:
          return validateTiming(formStore.propertyForm.timingAndAvailability);
        case 4:
          return validatePricing(formStore.propertyForm.bookingAndPricing);
        case 5:
  ...
  const validateBasicInfo = (data: any): boolean => {
    try {
      ...
      if (error.errors) {
        validationErrors.value[1] = error.errors.map((err: any) => err.message);
      }
      return false;
    }
  };
  ...
  const validatePropertyDetails = (data: any): boolean => {
    try {
      ...
      if (error.errors) {
        validationErrors.value[2] = error.errors.map((err: any) => err.message);
      }
      return false;
    }
  };
  ...
  const validateTiming = (data: any): boolean => {
    try {
      ...
      if (error.errors) {
        validationErrors.value[3] = error.errors.map((err: any) => err.message);
      }
      return false;
    }
  };
  ...
  const validatePricing = (data: any): boolean => {
    try {
      ...
      if (error.errors) {
        validationErrors.value[4] = error.errors.map((err: any) => err.message);
      }
      return false;
    }
  };
  ...
  const validateMedia = (data: any): boolean => {
    try {
      ...
      if (error.errors) {
        validationErrors.value[5] = error.errors.map((err: any) => err.message);
      }
      return false;
    }
  };
